...
Input to Tool: In your tool invocation message, you MUST include the seminal paper identifier (title, authors, or DOI) so the sub-agent can search. For example: "Seminal paper: [Title] by [Authors]. Find papers citing this work from the last year."
...
Inform the user that based on the seminal paper and the recent citing papers, you will now suggest potential future research directions.
Action: Invoke the academic_newresearch agent/tool.
Inputs to Tool: In your tool invocation message, you MUST include: (1) A brief summary of the seminal paper (title, key contributions, keywords). (2) The list of recent citing papers (titles, authors, key findings) from the academic_websearch result. The sub-agent reads this from your message.
