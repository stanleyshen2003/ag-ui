...
Monorepo: **backend** (AG-UI + ADK agent) and **frontend** (CopilotKit Next.js).

| Folder      | Description                    |
| ----------- | ------------------------------ |
| `backend/`  | Python AG-UI server, ADK agent, tests, static HTML chat |
...
<img src="backend/academic-research.svg" alt="academic researcher" width="800"/>
...
1.  **Prerequisites**
    ...
    * For local development: a [Google AI Studio](https://aistudio.google.com/app/apikey) API key (recommended).
    * For deployment to Vertex AI Agent Engine only: a Google Cloud project and [Google Cloud CLI](https://cloud.google.com/sdk/docs/install).

2.  **Installation**

    ```bash
    # Clone this repository and install backend dependencies.
    git clone <this-repo>
...
3.  **Configuration**
    ...
    *   **Using Google AI Studio (recommended for local development)**

        Get an API key from [Google AI Studio](https://aistudio.google.com/app/apikey), then set it in your environment or in a `.env` file:

        ```bash
        export GOOGLE_API_KEY=<your-google-ai-studio-api-key>
        ```

        Or copy `backend/.env.example` to `backend/.env` and fill in `GOOGLE_API_KEY`. No GCP project or `gcloud` login is required for local runs.

    *   **Using Vertex AI (GCP)** — optional, only needed for deployment to Agent Engine

        If you prefer Vertex AI or need to deploy to Agent Engine, set:
...
**Using with AG-UI (frontend)**

The agent is exposed via the AG-UI protocol using the **ag-ui-adk** wrapper (see `backend/main.py`). This allows HTML, CopilotKit, or other AG-UI clients to connect.

**Option A: CopilotKit frontend (recommended)**

Based on [CopilotKit’s AG-UI guide](https://www.copilotkit.ai/blog/build-a-frontend-for-your-adk-agents-with-ag-ui):

1. **Terminal 1** — start the AG-UI backend:
...
2. **Terminal 2** — from project root, install and run the Next.js + CopilotKit frontend:
...
3. Open **http://localhost:3000** in your browser. The CopilotKit sidebar connects to the backend at `http://localhost:8000` by default. Set `NEXT_PUBLIC_AG_UI_URL` in `frontend/.env.local` if your backend runs elsewhere.

**Option B: HTML frontend**

1. **Terminal 1** — start the AG-UI server:
...
2. **Terminal 2** — from project root, serve the static HTML chat (e.g. on port 8080):
...
3. Open **http://localhost:8080** in your browser. The page uses the API at `http://localhost:8000` by default; you can change the API base URL in the header if needed.

   On Windows you can start both from project root: `.\run_backend.ps1` (or from `backend/`: `.\run_ag_ui.ps1`).

**Note:** The AG-UI server in `backend/main.py` uses `ag_ui_adk.ADKAgent` to wrap `academic_research.root_agent` and `add_adk_fastapi_endpoint` to expose the agent over the AG-UI protocol. The agent code in `backend/academic_research/agent.py` is unchanged; only the server entry point uses AG-UI.
...
For running tests and evaluation, install the extra dependencies from the backend directory:
...
You may interact with the deployed agent using the `test_deployment.py` script (from `backend/`):
...
To delete the deployed agent, you may run the following command (from `backend/`):
